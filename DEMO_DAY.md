# StyleFlow BD Demo-Day Runbook

## Clean-machine setup (under 15 minutes)

```bash
git clone https://github.com/MahirAzizMoon/styleflow-bd-agent.git
cd styleflow-bd-agent
nvm use
corepack enable
...
The permanent public deployment is `https://styleflow-bd-agent.onrender.com`.
...
5. Run `pnpm start` again. If the quick tunnel URL changes, reopen the new URL.
